...
public:
  ...
  inline void rdObjectIds(OdDbObjectId* pRes, OdUInt32 num);
  inline OdDbObjectId rdObjectId();
  inline void wrObjectId(const OdDbObjectId& id);
};
...
inline void OdBlob::rdObjectIds(OdDbObjectId* pRes, OdUInt32 num)
{
  getBytes(pRes, num * sizeof(OdDbObjectId));
}

inline OdDbObjectId OdBlob::rdObjectId()
{
  OdDbObjectId id;
  getBytes(&id, sizeof(OdDbObjectId));
  return id;
}

inline void OdBlob::wrObjectId(const OdDbObjectId& id)
{
  putBytes(&id, sizeof(OdDbObjectId));
}
