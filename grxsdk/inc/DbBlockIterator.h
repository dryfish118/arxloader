...
public:
  ...
  /** \details
    Sets this Iterator object to reference the OdDbBlock that it would normally
    return first.
  ...
  */
  ...
  /** \details
    Returns the Object ID of the next object, and increments this Iterator
    object.
  */
  virtual OdDbObjectId next() = 0;
  
  /** \details
    Returns the Object ID of the record currently referenced by this Iterator
    object.
  */
 ...
 /** \details
    Returns an Iterator object that can be used to traverse the specified Block
    Table Record.
 ...
 */
 ...
 /** \details
    Returns an Iterator object that can be used to traverse queries defined by
    OdDbFilter objects applied to the specified Block Table Record.
 ...
 */
 ...
 /** \details
    Returns an Iterator object that can be used to traverse queries defined by
    an array of OdDbFilter objects applied to the specified Block Table Record.
...
/** \details
  This class implements Iterator objects that traverse queries defined by
  OdDbFilter objects on Block Table Records.
...
public:
  ...
  /** \details
    Returns the estimated fraction [0.0 .. 1.0] of the contents that will be
    selected by the OdDbFilter query.
  ...
  */
  ...
  /** \details
    Returns true if and only if the specified object passes the OdDbFilter query.
    
    \param objectId [in]  Object ID of the entity to be tested.
  */

  virtual OdResult accepts(OdDbObjectId objectId, OdBool& idPassesFilter) const = 0;

  /** \details
    Returns true if and only if the Index iterator, when not the primary
    iterator, is to buffer its output.
    
    \remarks
    When false, only the first index/filter pair is traversed, with subsequent
    pairs queried via accepts().

    If true, after all the IDs from the previous iterator have been added to the
    buffer, the start(), next() and id() of this iterator are to be used to
  ...
  */
  ...
  /** \details
    ...
    \sa
    <link OdDbFilteredBlockIterator::buffersForComposition@const, buffersForComposition()>
	\sa
...
/** \details
  This template class is a specialization of the OdSmartPtr class for
  OdDbFilteredBlockIterator object pointers.
*/
