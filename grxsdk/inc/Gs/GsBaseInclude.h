  ...
  public:
    ...
    */
    virtual OdUInt32 numVectorizers() const
    {
      ...
      {
        OdGsDevice *pDevice = OdGsBaseVectorizeView::getDevice();
    ...
    virtual OdGsBaseVectorizer* getVectorizer(bool /*bDisplay*/)
    {
      ...
      if (m_vectorizers.size() < m_vectUsage - 1)
      {
        OdGsViewPtr pView;
        {
          ...
          OdSaveState<OdLongPtr> _ss2(this->OdGsBaseVectorizeView::m_viewInfo.viewportId, (OdLongPtr)OdGsBaseVectorizeView::viewportId());
          pView.attach(OdGsBaseVectorizeView::clone(false));
          pView->setUserGiContext(OdGsBaseVectorizeView::userGiContext());
        }
...
#define MT_VECT_VIEWMTH_RET(mthSem, mthCall) \
    ...
    //MT_VECT_VIEWMTH(void setViewportBorderProperties(const OdGsColor& color, int width), setViewportBorderProperties(color, width))
    MT_VECT_VIEWMTH(void getViewportBorderProperties(OdGsColor& color, int& width) const, getViewportBorderProperties(color, width))
