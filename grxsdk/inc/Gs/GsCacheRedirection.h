...
class OdGsModelRedirectionWrapper : public Base
{
  ...
  public:
    ...
    */
    virtual void onAdded(OdGiDrawable* pAdded, OdIntDbId parentID)
    {
      for (OdUInt32 nModel = 0; nModel < m_pRedirectionClient->numModels(); nModel++)
    ...
    */
    virtual void onModified(OdGiDrawable* pModified, OdIntDbId parentID)
    {
      for (OdUInt32 nModel = 0; nModel < m_pRedirectionClient->numModels(); nModel++)
    ...
    */
    virtual void onErased(OdGiDrawable* pErased, OdIntDbId parentID)
    {
      for (OdUInt32 nModel = 0; nModel < m_pRedirectionClient->numModels(); nModel++)
    ...
    */
    ...
    {
      for (OdUInt32 nModel = 0; nModel < m_pRedirectionClient->numModels(); nModel++)
        m_pRedirectionClient->gsModelAt(nModel)->highlight(pGsNode, path, bDoIt, pView);
    }

    virtual bool highlight(const OdDbFullSubentPath& pPath, bool bDoIt = true, const OdGsView* pView = 0)
    {
      for (OdUInt32 nModel = 0; nModel < m_pRedirectionClient->numModels(); nModel++)
      {
        if (!m_pRedirectionClient->gsModelAt(nModel)->highlight(pPath, bDoIt, pView))
        {
          return false;
        }
      }
    ...
    */
    virtual void setSectioningVisualStyle(const OdDbObjectId visualStyleId)
    {
      for (OdUInt32 nModel = 0; nModel < m_pRedirectionClient->numModels(); nModel++)
    ...
    */
    ...
    GcGsGraphicsKernel &graphicsKernel(void)
    {
      return *((GcGsGraphicsKernel*)NULL);
    }

    bool addSceneGraphRoot(OdGiDrawable * pRoot)
    {
      return false;
    }

    bool eraseSceneGraphRoot(OdGiDrawable * pRoot)
    {
      return false;
    }
};
