...
public:
  ...
  */
  ...
  OdResult setBackground(const OdDbObjectId& background);

  OdDbObjectId visualStyle() const;
  OdResult setVisualStyle(const OdDbObjectId& visualStyle);

  bool isDefaultLightingOn() const;
  OdResult setDefaultLightingOn(bool isOn);

  OdGiViewportTraits::DefaultLightingType defaultLightingType() const;
  OdResult setDefaultLightingType(OdGiViewportTraits::DefaultLightingType lightingType);

  double brightness() const;
  OdResult setBrightness(double brightness);

  double contrast() const;
  OdResult setContrast(double contrast);

  OdCmColor ambientLightColor() const;
  OdResult setAmbientLightColor(const OdCmColor& color);

  OdDbObjectId sunId() const;
  OdDbObjectId setSun(OdDbSun* pSun);
  OdResult   setSun(OdDbObjectId& retId, OdDbObject* pSun);//Gcad(Modify)
  OdResult   setSun(OdDbObjectId& retId, OdDbObject* pSun, bool eraseOldSun);//Gcad(Modify)
