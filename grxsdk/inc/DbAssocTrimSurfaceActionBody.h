...
public:
	...
	*/
  explicit OdDbAssocTrimSurfaceActionBody(OdDbAssocCreateImpObject createImpObject = kOdDbAssocCreateImpObject);
  ...
  OdResult setTrimInfo(const OdArray<OdDbSurfaceTrimInfo> trimInfoArray, bool bAutoExtend);

  OdResult makeTrimPermanent();

  OdResult untrim();

  double getTrimmedArea() const;
  static OdResult createInstance(const OdDbObjectId& resultingSurfaceId, 
                                 const OdArray<OdDbSurfaceTrimInfo>& trimInfo,
                                 bool bAutoExtend, bool bEnabled,
                                 OdDbObjectIdArray& createdActionIds);
};
