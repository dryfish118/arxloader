...
// methods
 ...
 virtual PathErr Parse( LPCTSTR, const bool wildcard_ok = false );
        ...
        const CGdUiPathname& CGdUiPathname::operator=(const CGdUiPathname& pathSrc);
        const CGdUiPathname& CGdUiPathname::operator=(const CGdUiPathname* pathSrc);
 
        // generate a unique pathname based on this one.
        // returns a pointer to a new CGdUiPathname object if successful
        // returns FALSE if no success after 200 tries.
        BOOL CGdUiPathname::GetUniqueName(
                               CGdUiPathname&, 
        ...
        //  afterward.
        BOOL Open(FILE** fdp, const TCHAR *openmode);
        BOOL Open(CFile& fp, UINT openflags);

        BOOL OpenReadOnly(HANDLE &rFileHandle);
        BOOL OpenWriteOnly(HANDLE &rFileHandle);
        bool OpenReadOnly(Gstarsoft::GstarCAD::PAL::FileIO::File &);
        bool OpenWriteOnly(Gstarsoft::GstarCAD::PAL::FileIO::File &);
        ...
        //  afterward.
        BOOL Rename(const TCHAR *newname);
...
inline BOOL CGdUiPathname::WasAbsolute(void) const
{
