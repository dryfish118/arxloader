...
/////////////////////////////////////////////////////////////////////////////
// GdUi registered message
//
//  Notifications are usually sent via SendMessage() as follows:
//      message     set to the registered message, GdUiMessage().
//      wParam      HIWORD is GDUI_NOTIFY code, LOWORD varies with notification
//                  but is usually a control's ID (may be 0).
//      lParam      usually points to notification-specific data, may be NULL.
//
//  Handlers are expected to respond by at least returning an GDUI_REPLY,
//  where kIdUiReply_Nothing indicates the message was ignored, kIdUiReply_Ok
...
typedef enum {
    ...
    //  appropriate GDUI_REPLY code. lParam is a pointer to a CPoint
    ...
    // Draw Tip Text:
    //  lParam is a pointer to a CGdUiDrawTipText. The control should set up
    ...
    // Command state:
    //  LParam is a GDUI_COMMAND_STATE code. This notification is passed to
...
class GDUI_PORT CGdUiDrawTipText
{
public:
...
const UINT GDUI_PORT GdUiMessage ();

GDUI_NOTIFY GDUI_PORT GdUiNotification (WPARAM wp);

GDUI_REPLY GDUI_PORT GdUiNotify (CWnd& w, UINT id, GDUI_NOTIFY n, DWORD_PTR lParam);
GDUI_REPLY GDUI_PORT GdUiNotify (CWnd* w, UINT id, GDUI_NOTIFY n, DWORD_PTR lParam);

UINT GDUI_PORT GdUiNotifyId (WPARAM wp);

BOOL GDUI_PORT GdUiPostNotify (CWnd& w, UINT id, GDUI_NOTIFY n, DWORD_PTR lParam);
BOOL GDUI_PORT GdUiPostNotify (CWnd* w, UINT id, GDUI_NOTIFY n, DWORD_PTR lParam);
