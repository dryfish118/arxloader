...
GCDBCORE2D_PORT bool   isMSpaceActive(GcDbDatabase* pDb);// Internal use only. 
...
GCDB_DLL_DECL bool GSOFT_STDCALL gcdbDwkFileExists(const GCHAR *pszFileName);
GCDB_DLL_DECL int  GSOFT_STDCALL gcdbQueueForRegen(const GcDbObjectId *pIdArray, int nNumIds);

GCDB_DLL_DECL GcDbObjectId  gcdbGetDynDimStyleId( OdDbDatabase* pDb = NULL );

GCDB_DLL_DECL bool gcdbGetPreviewBitmapFromDwg(const GCHAR* pszDwgfilename, HBITMAP* pPreviewBmp, HPALETTE* pRetPal);
GCDB_DLL_DECL bool gcdbDisplayPreviewFromDwg(const GCHAR * pszDwgfilename,void* pPreviewWnd,const Gsoft::UInt32* pBgColor = NULL);
...
// Internal use only.
GCDBCORE2D_PORT Gcad::ErrorStatus
