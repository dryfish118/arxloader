...
GCED_DLL_DECL int gcedUpdate(int vport, gds_point p1, gds_point p2);
...
GCED_DLL_DECL int gcedDefunEx(const GCHAR* pszGlobalName, const GCHAR* pszLocalName, int nFuncNum);
...
GCED_DLL_DECL int gcedFindTrustedFile(const GCHAR* fname, GCHAR* result, size_t nBufLen);
...
#ifdef __cplusplus
GCED_DLL_DECL int gcedSSLength  (const gds_name sname, Gsoft::Int32 *len);
#else
...
GCED_DLL_DECL int gds_queueexpr(const GCHAR* expr);
