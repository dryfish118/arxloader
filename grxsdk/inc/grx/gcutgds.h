...
GCUT_DLL_DECL int	 gcutSPrintf(GCHAR *buffer, size_t nBufLen, const GCHAR *format, ...);
GCUT_DLL_DECL int  gcutVSPrintf(wchar_t *buf, size_t nBufLen, const wchar_t *fmt, va_list va);

template<size_t nBufLen>
int  gcutVSPrintf(wchar_t(&buf)[nBufLen], const GCHAR *format, va_list va);
...
GCUT_DLL_DECL void gcad__assert(const GCHAR *condition, const GCHAR *filename, int lineNumber);
GCUT_DLL_DECL int  gcutRbDup(const struct resbuf *source, struct resbuf **result);
