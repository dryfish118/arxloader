...
GCAD_DLL_DECL TaskId gcedExecuteOnMainThread(const GcedRunLoopFn pfn, void* pContext, bool bWaitUntilDone = false);
...
GCAD_DLL_DECL TaskId gcedExecuteOnMainThread(IGcEdRunLoopTask* pTask, bool bWaitUntilDone = false);


GCAD_DLL_DECL void gcedRemoveRunLoopTask(TaskId taskId);

typedef HBITMAP (* GcedCommandBitmapQueryFn)(const TCHAR* cmdName, 
                                             const COLORREF& backgroundColor);

GCAD_DLL_DECL BOOL      gcedRemoveCommandBitmapQuery   (const GcedCommandBitmapQueryFn pfn); 


GCAD_DLL_DECL BOOL      gcedRegisterCommandBitmapQuery (const GcedCommandBitmapQueryFn pfn);

GCAD_DLL_DECL HBITMAP   gcedGetBitmapForCommand        (const TCHAR* cmdName,
                                                      const COLORREF& backgroundColor);

GCAD_DLL_DECL HBITMAP   gcedGetCachedBitmapForCommand  (const wchar_t* cmdName, 
                                                      const COLORREF& backgroundColor);
GCAD_DLL_DECL bool gcedGetToolTipInfoForCommand( const GCHAR* cmdName
