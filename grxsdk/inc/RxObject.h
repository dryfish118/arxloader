...
#define ODRX_DEFINE_INIT_MEMBERS_GENERIC(ClassName, ParentClass, CREATE_CLASS_INSTANCE, CREATE_CLASS_INSTANCE2)   \
                                                                                                     \
...
ODRX_DEFINE_MEMBERS2(ClassName,ParentClass,ClassName::pseudoConstructor,VerNo,0,0,OD_T(#ClassName),OdString::kEmpty,OdString::kEmpty,0) \
...
public:
  ...
  */
  ODRX_SEALED_VIRTUAL OdRxObjectPtr clone(ODSYSDEF) const ODRX_SEALED;
 ...
 */
  ...
  */
  virtual bool isEqualTo(
    const OdRxObject* pOther) const;
 protected:
  virtual OdRxObject* subQueryX(const OdRxClass* protocolClass) const;
};
...
protected:
  ...
  OdRxClass(const OdChar* name, const OdChar* parent, OdRxMemberCollectionConstructorPtr memberConstruct, void* userData = NULL);
public:
  ...
  */
  ODRX_SEALED_VIRTUAL OdRxObjectPtr addX(ODSYSDEF,
    OdRxClass* pProtocolClass, 
    OdRxObject* pProtocolObject) ODRX_SEALED;
...
//     const OdRxClass* pProtocolClass);
  ...
  /** \details
    ...
    It will be fully implemented in a future *release*.
  */
  ODRX_SEALED_VIRTUAL OdRxObjectPtr delX(ODSYSDEF,
    OdRxClass* pProtocolClass) ODRX_SEALED;
  ...
  /** \details
    ...
    <link rtti_pseudoconstructor.html, Understanding Pseudo-Constructors>
  */
  ODRX_SEALED_VIRTUAL OdRxObjectPtr create(ODSYSDEF) const ODRX_SEALED;
  virtual OdRxObject* create() const;
  ...
  */
  ...
  static bool isDerivedFrom(const OdRxClass *pDerived, const OdRxClass* pBase);

  const OdRxSet* descendants() const;

  void odrxInitTypeSystem();
  /** \details
...
{
  OdRxObject* pObj = 0;
  if (!pClass || !Class::desc()) // requested class is not registred
    return pObj;
...
FIRSTDLL_EXPORT OdRxClass* newOdRxClass(
  ...
  int         maintVer,
  int         proxyFlags,
  OdRxObject* (*pseudoConstructor)(),
  const OdChar* dxfName,
  const OdChar* appName,
  AppNameChangeFuncPtr func,
  OdRxMemberCollectionConstructorPtr makeMembers,
  void* userData = NULL);
...
FIRSTDLL_EXPORT void deleteOdRxClassDefault(OdRxClass* pClass);

FIRSTDLL_EXPORT void odrxInitTypeSystem();
FIRSTDLL_EXPORT void odrxTerminateTypeSystem();
...
FIRSTDLL_EXPORT OdRxClass* odrxQueueClassForInitialization(OdRxClass** pgpDesc, RxInitFuncPtr rxInit);
FIRSTDLL_EXPORT void odrxInitializeClassObjects();      // for internal use only
