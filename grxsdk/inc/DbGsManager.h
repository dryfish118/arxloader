...
public:
  ...
  */
  enum ViewportFilter
  {
    kVpScreenFilter = (1 << 0), // Check does viewport is visible on screen.
    kVpSizeFilter   = (1 << 1)  // Check does viewport dimensions larger corresponding value.
  };
  /** \details
    Viewport filter branch.
  */
  enum ViewportFilterBranch
  {
    kVpBrDefault = 1, // Default branch.
    kVpBrPlot    = 2, // Plot generation branch.
    kVpBrAll     = 3  // All viewport filter branches.
  };
  /** \details
    Viewport filter application mode.
  */
  enum ViewportFilterMode
  {
    kVpFilterVisible   = 3, // Apply filter with visible effect (default).
    kVpFilterInvisible = 1, // Apply filter without visible effect.
    kVpFilterDisable   = 0  // Avoid filter application.
  };
  /** \details
    Setup specified viewport filter parameters.
    \param vpFilter [in]  Specify filter type for parameters setup.
    \param filterBranch [in]  Specify filter branch(es) for parameters setup.
    \param filterMode [in]  Specify filter application mode.
    \param filterParam [in]  Specify filter parameter (actual for size filter only, where it is mean minimal viewport size in pixels).
  */
  virtual void setViewportFilter(ViewportFilter vpFilter, ViewportFilterBranch filterBranch = kVpBrAll,
