...
inline OdDbWipeoutVariablesPtr OdDbWipeoutVariables::openWipeoutVariables(OdDbDatabase* pDatabase, OdDb::OpenMode mode) {
  ...
  if (pDatabase) {
    ...
    OdString strAcDbWipeoutVariables(L"AcDbWipeoutVariables");
    pWpVars = ::odrxCastByClassName<OdDbWipeoutVariables>(pRootDict->getAt(ACAD_WIPEOUT_VARS).openObject(mode), strAcDbWipeoutVariables);
    if (pWpVars.isNull() && mode == OdDb::kForWrite) {
      pRootDict->upgradeOpen();
      pWpVars = ::odrxSafeCreateObject<OdDbWipeoutVariables>(strAcDbWipeoutVariables);
