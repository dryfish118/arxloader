...
public:
  ...
  */
  ...
  template<class ObjType> OdResult getObject(ObjType *& pObject,
    OdDb::OpenMode mode = OdDb::kForRead)
  {
    const OdDbObjectId id = this->objectId();
    return ::oddbOpenObject(pObject, id, mode, /*openErased*/false);
  }

  virtual OdDbObjectPtr getObject(
    OdDb::OpenMode openMode = OdDb::kForRead);

  OdResult getObject(OdDbObject*& pObject, OdDb::OpenMode openMode = OdDb::kForRead)
  {
    return this->getObject<OdDbObject>(pObject, openMode);
  }
  /** \details
...
/** \details
    ...
    * Entry names honor the rules of Symbol names:
    ...
    \sa
    <link OdDbDictionaryIterator, OdDbDictionaryIterator class>
