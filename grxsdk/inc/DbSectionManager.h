...
public:
  ...
  OdDbSectionManager();
  virtual ~OdDbSectionManager();

  OdDbObjectId getSection(const OdString& pszName ) const;
  OdResult getSection(const OdChar * name, OdDbObjectId& id) const;
  OdDbSectionPtr getSection(const OdString& pszName, OdDb::OpenMode mode) const;
  OdResult getSection(const OdChar * name, OdDbSection*& pSecPlane, OdDb::OpenMode mode) const;
  OdDbObjectId getLiveSection() const;
  OdResult getLiveSection(OdDbObjectId& id) const;
  OdDbSectionPtr getLiveSection(OdDb::OpenMode mode) const;
  OdResult getLiveSection(OdDbSection*& pSecPlane, OdDb::OpenMode mode) const;
  int numSections() const;
  OdDbSectionManagerIteratorPtr newIterator () const;
  OdResult newIterator(OdDbSectionManagerIterator*& pIterator) const;

  OdResult getUniqueSectionName(const OdString& pszBaseName, 
    OdString& pszUniqueName) const;

  OdResult getUniqueSectionName(const OdChar * baseName, OdChar*& pszUniqueName) const;
...
public:
  ...
  OdResult getSection (OdDbSection*& pSecPlane, OdDb::OpenMode openMode) const;
protected:
  OdDbSectionManagerIterator(OdDbSectionManager* pMgr);
};
