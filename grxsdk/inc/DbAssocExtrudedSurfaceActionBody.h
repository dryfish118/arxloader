...
public:
	...
	*/
  explicit OdDbAssocExtrudedSurfaceActionBody(OdDbAssocCreateImpObject createImpObject = kOdDbAssocCreateImpObject);
  ...
  OdResult setTaperAngle(double dTaperAngle, const OdString& expression = OdString(),
    const OdString& evaluatorId = OdString());

  double taperAngle(OdString& expression = dummyString(), OdString& evaluatorId = dummyString()) const;

  OdResult setHeight(double dHeight, const OdString& expression = OdString(),
    const OdString& evaluatorId = OdString());

  double height(OdString& expression = dummyString(), OdString& evaluatorId = dummyString()) const;

  static OdResult createInstance(const OdDbObjectId& resultingSurfaceId,
    const OdDbPathRef& inputPath,
