...
class OdGsDevice;
class OdDbGlobals;
class OdDbInternalServices;
class OdDbInternalServices2;
...
typedef OdResult(*SELECTFILEPROC)(/*[out]*/ short *userCancel,
  /*[out]*/ OdChar *& chosenPath,
  void* h, // HWND
  const int nFlags,
  const OdChar * prompt,
  const OdChar * dir,
...
typedef OdResult(*REMAPPROC) (/*[out]*/ OdChar *& newName,
  const OdChar * filename,
  int context,
  void* pReserved);

typedef void(*MAPFILENOTIFYPROC) (const OdChar * filename,
...
public:
  ...
  enum FindFileHint
  {
    ...
    kPlotStyleFile        = 13,  // Plot style file.
  ...
  */
  virtual void auditPrintReport(
    ...
    const OdChar* strLine,
    int printDest) const;


  virtual void auditPrintReport(OdDbAuditInfo* pAuditInfo, const OdChar* strLine, int printDest) const;
  /** \details
  ...
  */
  ...
  /** \details
    ...
    Font Handling
  */
  virtual const OdChar* getAlternateFontName() const = 0;
  ...
  */
  virtual const OdChar* getFontMapFileName() const = 0;
