...
/** \details
  ...
  \sa
  <link db_datasheet_sample_root, Example of Selecting a Predefined Table Object>
  ...
  \sa
  <link db_datasheet, Working with Predefined Tables of Named Records>
...
public:
  ...
  */
  ...
  /** \details
    ...
    \sa
    <link db_datasheet_get_check, Getting and Checking Records>
  */
  ...
  */
  ...
  /** \details
    ...
    \sa
    <link db_datasheet_iterate, Iterating through Records>
  */
  ...
  /** \details
    ...
    \sa
    <link db_datasheet_add_delete, Adding, Naming, Deleting, Recovering Records>

    OdError_DuplicateRecordName class
  */
  virtual OdDbObjectId add(ODSYSDEF, OdDbSymbolTableRecord* pRecord);

  OdResult add(
    OdDbObjectId& recordId,
    OdDbSymbolTableRecord* pRecord);

  OdResult add(OdDbSymbolTableRecord* pRecord);
  ...
  protected:
    template<class RecordType> OdResult openAt(const OdChar* entryName,
      RecordType * & pRec, OdDb::OpenMode nMode, bool bOpenErased) const
    {
      pRec = nullptr;
      OdDbObjectId id;
      OdResult es = this->getIdAt(entryName, id, bOpenErased);
      if (es == eOk)
        es = ::oddbOpenObject(pRec, id, nMode, bOpenErased);
      return es;
    }
};
...
public:
  ...
  */
  ...
  OdResult getRecordId(OdDbObjectId& id) const;
...
protected:
  template<class RecordType> OdResult openRecord(RecordType * & pRec,
    OdDb::OpenMode nMode, bool bOpenErased) const
  {
    pRec = nullptr;
    OdDbObjectId id;
    OdResult es = this->getRecordId(id);
    if (es == eOk)
      es = ::oddbOpenObject(pRec, id, nMode, bOpenErased);
    return es;
  }
