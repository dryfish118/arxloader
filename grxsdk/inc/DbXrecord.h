...
#define ODDB_XRECORD_CLASS          OD_T("AcDbXrecord")
#define ODDB_XRECORD_ITERATOR_CLASS OD_T("AcDbXrecordClass")
...
public:
  ...
  */
  ...
  OdResult getCurResbuf(resbuf& outItem, OdDbDatabase* db) const;
  OdResult modifyCurResbuf(const resbuf& item);
  OdResult insertRbChain(const resbuf& pRb);
  OdResult removeCurResbuf();
protected:
  OdDbXrecordIterator() {}
  OdDbXrecordIterator(
...
public:
  ...
  */
  ...
  OdResult appendRbChain(const resbuf& pRb);
