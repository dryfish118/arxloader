...
public:
  ...
  virtual void reportUnhandledArxException(const OdChar* pExceptionDescription, const OdChar *pAppName);
  virtual const OdChar* releaseMarketVersion();
  ...
  virtual bool _entToWorldTransform(double normal[3],
    flagmat *tran);

  virtual void terminateScript();

  virtual OdResult serializeOleItem(COleClientItem* pItem,
    CArchive*);
  ...
  virtual bool requiredVersionNotAvailable(RequiredVersionFlags flags,
    OdDbDatabase* pDb) const;

  template<size_t N> bool isRemoteFile(const OdChar * pszLocalFile,
    OdChar(&pszURL)[N]) const
  {
    return isRemoteFile(pszLocalFile, pszURL, N);
  }

  virtual bool isRemoteFile(const OdChar* pszLocalFile, OdChar* pszURL, size_t urlLen) const;

  OdString getTempPath();

  virtual bool getSubstituteFont(OdChar **pFileName, const OdChar* prompt, int type, int fontAlt);

  virtual OdResult getRemoteFile(const OdChar* pszURL, OdChar* pszLocalFile, size_t localFileLen, bool bIgnoreCache) const;

  virtual bool getPassword(const OdChar* dwgName, PasswordOptions options, wchar_t* password, const size_t bufSize);

  virtual bool notifyCorruptDrawingFoundOnOpen(OdDbObjectId id, OdResult es);
  ...
  OdResult mapInetStatusToAcadStatus(const OdInet::Status status) const;

  virtual OdResult mapFile(OdChar *& newName, const OdChar* filename, int context, void* pReserved);

  virtual bool loadApp(const OdChar* appName, OdaApp::LoadReasons why, bool printit, bool asCmd);

  template<size_t N> bool launchBrowserDialog(
    OdChar(&pszSelectedURL)[N],
  ...
  virtual bool launchBrowserDialog(OdChar* pszSelectedURL, size_t selectedUrlLen, const OdChar* pszDialogTitle,
    const OdChar* pszOpenButtonCaption, const OdChar* pszStartURL, const OdChar* pszRegistryRootKey = 0, bool bOpenButtonAlwaysEnabled = false) const;

  virtual bool isCloudFile(const OdChar* pszCloudFile) const;
  ...
  virtual OdLocale     getProductLocale();

  virtual OdResult getNewOleClientItem(COleClientItem*& pItem);

  virtual OdDbBaseHostAppServices::ModelerFlavor getModelerFlavor() const;

  virtual short getKeyState(int nVirtKey) const;

  virtual OdResult getGraphicsCacheFolder(const OdChar*& folder);

  virtual bool getDefaultPlotCfgInfo(OdChar* plotDeviceName, size_t nDevNameLen, OdChar* plotStyleName, size_t nStyleNameLen);

  bool getDefaultPlotCfgInfo(OdChar(&devName)[32], OdChar(&styleName)[32]);

  void getDefaultPlotCfgInfo(OdString& devName, OdString& styleName);

  virtual OdResult getAllUsersRootFolder(const OdChar*& folder);

  virtual OdDbInternalServices* gcadInternalServices();

  virtual void fatalError(const OdChar* format, ...);

  virtual void enableMessageDisplay(bool bEnableMessageDisplay);
  virtual void drawOleOwnerDrawItem(COleClientItem* pItem, OdLongPtr hdc, OdInt32 left, OdInt32 top, OdInt32 right, OdInt32 bottom);
  ...
  void acadInternalSetvar(const OdChar* name, void* var);

  virtual OdDbInternalServices2* acadInternalServices2();

  void* acadInternalGetvar(const OdChar *name);

  virtual OdDbPlotSettingsValidator* plotSettingsValidator() const;

  virtual void auditPrintReport(OdDbAuditInfo* pAuditInfo, const OdChar* line, int both) const;

  inline OdDbAppSystemVariables* workingAppSysvars() const;

  inline void setWorkingAppSysvars(OdDbAppSystemVariables* pSysvars);

  inline code_page_id
    getSystemCodePage() const
  {
    return code_page_id(0);
  }
  /** \details
  ...
  */
  ...
  virtual OdResult findFile(
    OdChar* pcFullPathOut,
    int nBufferLength,
    const OdChar* pcFilename,
    OdDbDatabase* pDb = NULL,
    OdDbBaseHostAppServices::FindFileHint hint = kDefault);

  virtual OdResult findFile(OdString& fileOut,
    const OdChar * pcFilename,
    OdDbDatabase * pDb = nullptr, // When this search 
                                  // is related to a db
    OdDbBaseHostAppServices::FindFileHint hint = kDefault);

  virtual OdResult getRemoteFile(
    const OdChar * pszURL,
    OdString pszLocalFile,
  ...
  */
  virtual void auditPrintReport(OdAuditInfo* pAuditInfo, const OdChar* strLine, int printDest) const;
  /** \details
...
#undef REGVAR_DEF
  ...
  virtual const OdChar* getAlternateFontName() const;
  ...
  */
  virtual const OdChar* getFontMapFileName() const;
  ...
  */
  ...
  virtual OdResult brepBuilder(OdBrepBuilder& brepBuilder, BrepType bbType)
  {
    ...
    if (!pService.isNull())
    {
      OdModelerGeometryCreatorPtr pCreator = (OdModelerGeometryCreator*)pService->ODSYSCALL0(create).get();
      if (!pCreator.isNull())
      {
...
private:
  ...
  virtual OdDbUndoController *defaultUndoController();
  
  virtual OdDbTransactionManager* workingTransactionManager();
  virtual unsigned int getTempPath(OdUInt32, OdChar *) { return 0; }
...
TOOLKIT_EXPORT OdResult oddbSetHostApplicationServices(OdDbInternalServices * pServices);
TOOLKIT_EXPORT OdResult oddbSendInitialDwgFileOpenComplete(OdDbDatabase* pDb);
TOOLKIT_EXPORT void oddbTriggerAcadOctTreeReclassification();
TOOLKIT_EXPORT OdResult oddbSetBreak3dQueueForRegen(bool);
TOOLKIT_EXPORT OdDbGlobals* getDefaultGlobals();
