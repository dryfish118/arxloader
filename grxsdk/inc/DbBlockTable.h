...
/** \details
  This class implements bidirectional Iterator objects that traverse entries in
  OdDbBlockTable objects in an OdDbDatabase instance.
...
/** \details
  ...
  OdDbBlockTableIterator object pointers.
*/
typedef OdSmartPtr<OdDbBlockTableIterator> OdDbBlockTableIteratorPtr;


/** \details
  This class implements the BlockTable, which represents block definitions in an
...
public:
 ...
 OdResult getAt (
  ...
  /** \details
    Determines whether the named record object exists in the predefined table
    object using the specified name.

    \param recordName [in] Record name.

    \returns
    Returns true if and only if the predefined table object contains the
    specified named record object.
  */
  virtual bool has(const OdChar* recordName) const;

  /** \details
    Determines whether the named record object exists in the predefined table
    object using the specified object ID.

    \param objectId [in] Object ID.

    \returns
    Returns true if and only if the predefined table object contains the
    specified record object.
  */
  virtual bool has(const OdDbObjectId& objectId) const;

  /** \details
    Returns an Iterator object that can be used to traverse through record
    objects in the predefined table object.

    \param atBeginning [in] True &ndash; to start at the beginning, or
                            false &ndash; to start at the end.
    \param skipDeleted [in] True &ndash; to iterate only unerased records, or
                            false &ndash; to iterate through unerased and erased records.
  */
  ...
  */
  ...
  OdResult add(OdDbObjectId& recordId, OdDbBlockTableRecord* pRecord);

  OdResult add(OdDbBlockTableRecord* pRecord);

  /** \details
    Returns the Object ID of the Model Space record within this BlockTable
    object.
  */
  const OdDbObjectId& getModelSpaceId() const;

  /** \details
    Returns the Object ID of the Paper Space record within this BlockTable
    object.
  */
...
/** \details
  This template class is a specialization of the OdSmartPtr class for
  OdDbBlockTable object pointers.
*/
