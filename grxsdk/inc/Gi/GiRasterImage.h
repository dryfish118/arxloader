...
#include "../RxObject.h"
#include "../Gi/ExGiImage.h"  //Added by xubo 20210409 for ÖØ¹¹¹âÕ¤Í¼Ïñ¿â
...
public:
  ...
  */
  ...
  /* BEGIN: Added by xubo 20210115 for ÖØ¹¹¹âÕ¤Í¼Ïñ¿â */
  typedef enum ePixelMode
  {
    kChlDefault,
    kChlRGB,
    kChlBGR,
  ...
  typedef struct tagColorFormatInfo
  {
    int backRed;              //±³¾°ÑÕÉ«µÄºìÉ«·ÖÁ¿
    int backGreen;            //±³¾°ÑÕÉ«µÄÂÌÉ«·ÖÁ¿
    int backBlue;             //±³¾°ÑÕÉ«µÄÀ¶É«·ÖÁ¿
    int foreRed;              //Ç°¾°ÑÕÉ«µÄºìÉ«·ÖÁ¿
    int foreGreen;            //Ç°¾°ÑÕÉ«µÄÂÌÉ«·ÖÁ¿
    int foreBlue;             //Ç°¾°ÑÕÉ«µÄÀ¶É«·ÖÁ¿
    int lightGain;            //ÁÁ¶ÈÔöÒæ£º128ÑÕÉ«²»±ä£»0ºÚÉ«£»256´¿°×É«
    int saturationGain;       //¶Ô±È¶ÈÔöÒæ£º128ÑÕÉ«²»±ä£»0×îÐ¡¶Ô±È¶È£»256×î´ó¶Ô±È¶È
    int fade;                 //ÍËÉ«¶È£º0ÑÕÉ«²»±ä£»256ËùÓÐÑÕÉ«¶¼ÊÇ±³¾°ÑÕÉ«
    int alphaBlend;           //°ëÍ¸Ã÷¶È
    bool transparent;         //ÊÇ·ñ±³¾°Í¸Ã÷
  } ColorFormatInfo;

  typedef struct tagAffineInfo
  {
    POINT *ptPlg;             //·ÂÉä±ä»»×ø±êÏµ ¶ÔÓ¦Ô´Í¼ÏñµÄ{(0, 0), (width, 0), (0, height)}
    POINT *ptClip;            //·ÂÉä±ä»»×ø±êÏµ ¶ÔÓ¦Ô´Í¼ÏñµÄ{(left, top), (right, top), (left, height)}
    RECT *pSrcClipRect;       //Ô´Í¼Ïñ{(left, top), (right, top), (left, height)}
    RECT *pRect;              //Ä¿±ê±ÕºÏÇøÓò
    double **pAffineMatrix;   //·ÂÉä±ä»»¾ØÕó
    OdUInt32 dstWidth;        //Ä¿±êÍ¼Ïñ¿í¶È
    OdUInt32 dstHeight;       //Ä¿±êÍ¼Ïñ¸ß¶È
    int dstCco;               //Ä¿±êÍ¼ÏñÏñËØÄ£Ê½
    int dstBpp;               //Ä¿±êÍ¼ÏñÎ»Éî¶È
  } AffineInfo;
  /* END: Added by xubo 20210115 for ÖØ¹¹¹âÕ¤Í¼Ïñ¿â */
  ...
  */
  virtual OdUInt8 defaultResolution(double& xPelsPerUnit, double& yPelsPerUnit) const;
  ...
  */
  ...
  /* BEGIN: Added by xubo 20201009 for 重构光栅图像库 */
  virtual bool isImgValid() const { return false; }
  virtual void informUseCvtDib(const bool bUse) { return; }
  virtual bool isUseCvtDib() const { return false; }
  virtual bool isSupportDecoding(const wchar_t *pFullName, const wchar_t *pExtName, int * const pImgType) const { return false; }
  virtual GStar::Image* convertToStandardImage
  (
    const OdGeVector2d &origin,
    const OdGeVector2d &u,
    const OdGeVector2d &v,
    const ColorFormatInfo *pClrInfo,
    const OdGePoint2dArray &ClipPts,
    const OdUInt32 ClipPtNum,
    const OdGePoint2dArray &RegionPts,
    const OdUInt32 RegionPtNum,
    BITMAPINFO ** const ppBMI,
    void ** const ppBitsOut,
    OdGeVector2d &orgOffset,
    bool isbMonochrome = false
  ) { return NULL; }
  /* END: Added by xubo 20201009 for 重构光栅图像库 */
};
