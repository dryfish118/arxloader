...
public:
  ...
  */
  enum SetAttributesFlags
  {
    ...
    kDrawableRegenTypeDependentGeometry   = 64, // This flag must be set for objects which is drawn differently in different regen type modes.
    ...
    kDrawableDBRO                         = 131072, 

    kLastFlag                             = kDrawableDBRO // Marker
  };
  ...
  */
  virtual void setGsNode(OdGsCache* pGsNode) { pGsNode; }
  ...
  */
  virtual OdGsCache* gsNode() const { return nullptr; }
  virtual OdBool  RolloverHit(OdULongPtr nSubentId, OdULongPtr nMouseFlags, OdBool bReset);
 ...
 */
  ...
  virtual void setDrawStream(OdGiDrawStream * pStream);
  virtual OdGiDrawStream* drawStream(void) const;
  protected:
...
//==================================Add========================
class FIRSTDLL_EXPORT OdGiDrawableReactor : public OdRxObject
{
public:
  ODRX_DECLARE_MEMBERS(OdGiDrawableReactor);
  ...
  virtual void beginRegen(OdGiDrawable*, RegenFlags);
  virtual void endRegen(OdGiDrawable*, RegenFlags);
  virtual void modified(OdGiDrawable*);
  virtual void erased(OdGiDrawable*);
  virtual void handOverTo(OdGiDrawable*, OdGiDrawable*);
  virtual void erased(OdGiDrawable*, OdLongPtr);
  virtual void modified(OdGiDrawable*, OdLongPtr);
  virtual void added(OdGiDrawable*, OdLongPtr, OdLongPtr);
};

#define odgiDrawableEvent (&OdGiDrawableEvent::instance())

class  OdGiDrawableEvent : public OdRxObject
{
public:
  ODRX_DECLARE_MEMBERS(OdGiDrawableEvent);


  FIRSTDLL_EXPORT static OdGiDrawableEvent& instance();

  virtual void addReactor(OdGiDrawableReactor* pReactor);

  virtual void removeReactor(OdGiDrawableReactor* pReactor);

  virtual void sendBeginRegen(OdGiDrawable* pDrawable, OdGiDrawableReactor::RegenFlags flags);

  virtual void sendEndRegen(OdGiDrawable* pDrawable, OdGiDrawableReactor::RegenFlags flags);

  virtual void sendModified(OdGiDrawable* pDrawable);

  virtual void sendErased(OdGiDrawable* pDrawable);

  virtual void sendHandOverTo(OdGiDrawable* pDrawableFrom, OdGiDrawable* pDrawableTo);

  virtual void sendErased(OdGiDrawable* pDrawable, OdLongPtr parentID);

  virtual void sendModified(OdGiDrawable* pDrawable, OdLongPtr parentID);

  virtual void sendAdded(OdGiDrawable* pDrawable, OdLongPtr parentID, OdLongPtr databasePtr);

private:
  OdGiDrawableEvent();
  OdGiDrawableEvent(const OdGiDrawableEvent&);
  void operator=      (const OdGiDrawableEvent&);
  ~OdGiDrawableEvent();

  class OdGiImpDrawableEvent* m_pImp;
};
