...
class ODGI_EXPORT OdGiMaterialTextureLoadPE : public OdRxObject
{
  public:
    ...
    */
    virtual void startTextureLoading(OdString &fileName, OdDbDatabase* pDb) = 0;
    ...
    /** \details
      ...
      \param pDb [in] Pointer to the database.
    */
    virtual void textureLoaded(const OdString &fileName, OdDbDatabase* pDb) = 0;
    /** \details
      Called by texture loader after file loading, only if texture loading failed.
      \param fileName [in] File name of the texture.
      \param pDb [in] Pointer to the database.
    */
    virtual void textureLoadingFailed(const OdString &fileName, OdDbDatabase* pDb) = 0;
};
