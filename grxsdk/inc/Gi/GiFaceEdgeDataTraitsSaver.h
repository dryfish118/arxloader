...
#endif // EX_TRAITS_SELMARK_RESTORE
  protected:
    OdGiFaceEdgeDataTraitsSaverBase(OdGiDrawableTraits *pTraits, OdGiConveyorContext *pDrawCtx, OdGsView::RenderMode renderMode)
      : m_pTraits(pTraits), m_pDrawCtx(pDrawCtx), m_renderMode(renderMode), m_components(0)
    {
      m_defaultLayerId = m_pTraits->layerId();
    }
    ...
    bool setColor(const short* pColors, int index)
    {
    ...
    bool setLayer(OdDbObjectId*pLayers, int index)
    {
      if (m_curLayerId != (OdDbStub*)pLayers[index])
      {
...
class OdGiFaceDataTraitsSaver : protected virtual OdGiFaceEdgeDataTraitsSaverBase
{
  ...
  public:
    OdGiFaceDataTraitsSaver(const OdGiFaceData *pFaceData, OdGiDrawableTraits *pTraits, OdGiConveyorContext *pDrawCtx,
      ...
      : OdGiFaceEdgeDataTraitsSaverBase(pTraits, pDrawCtx, renderMode), m_pFaceData(pFaceData)
    {
      if (pFaceData && pTraits)
      {
        ...
        {
          EX_TRAITS_SET(m_prevLayerId, m_curLayerId, pTraits->layerId());
        ...
        if (!bForEdge)
        {
          ...
          {
            EX_TRAITS_SET(m_prevMaterialId, m_curMaterialId, pTraits->materialId());
    ...
    bool setFaceTraits(int faceIndex)
    {
      ...
      if (m_components & kFaceSetMask)
      {
        ...
        {
          OdDbObjectId*pMaterials = m_pFaceData->materials();
...
class OdGiEdgeDataTraitsSaver : protected virtual OdGiFaceEdgeDataTraitsSaverBase
{
  ...
  public:
    OdGiEdgeDataTraitsSaver(const OdGiEdgeData *pEdgeData, OdGiDrawableTraits *pTraits, OdGiConveyorContext *pDrawCtx,
      ...
      : OdGiFaceEdgeDataTraitsSaverBase(pTraits, pDrawCtx, renderMode), m_pEdgeData(pEdgeData)
    {
      if (pEdgeData && pTraits)
      {
        ...
        {
          EX_TRAITS_SET(m_prevLinetypeId, m_curLinetypeId, pTraits->lineTypeId());
    ...
    bool setEdgeTraits(int edgeIndex)
    {
      ...
      if (m_components & kEdgeSetMask)
      {
        ...
        {
          OdDbObjectId*pLinetypes = m_pEdgeData->linetypeIds();
          if (m_curLinetypeId != (OdDbStub *)pLinetypes[edgeIndex])
          {
...
class OdGiFaceEdgeDataTraitsSaver : public OdGiFaceDataTraitsSaver, public OdGiEdgeDataTraitsSaver
{
  public:
    OdGiFaceEdgeDataTraitsSaver(const OdGiFaceData *pFaceData, const OdGiEdgeData *pEdgeData,
                                OdGiDrawableTraits *pTraits, OdGiConveyorContext *pDrawCtx,
