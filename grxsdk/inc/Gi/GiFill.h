...
class OdGiImpHatchPatternDefinition;
...
inline
OdResult OdGiFill::copyFrom(const OdRxObject* pSource)
{
  OdGiFillPtr pSrcFill = OdGiFill::ODSYSCALL1(cast, pSource);
  if (!pSrcFill.isNull())
  {
...
class FIRSTDLL_EXPORT OdGiHatchPatternDefinition
{
public:
  OdGiHatchPatternDefinition();
  OdGiHatchPatternDefinition(const OdGiHatchPatternDefinition &def);
  OdGiHatchPatternDefinition(double angle,
    double baseX,
    double baseY,
    double offsetX,
    double offsetY,
    const OdGeDoubleArray& dashList);
  ~OdGiHatchPatternDefinition();
  ...
  const OdGeDoubleArray& dashList() const;

  OdGiHatchPatternDefinition & operator = (const OdGiHatchPatternDefinition &def);

private:
  OdGiImpHatchPatternDefinition *  mpImp;
};
...
public:
  ...
  double m_offsetX;
  double m_offsetY;
  OdGeDoubleArray m_dashList;

  OdGiImpHatchPatternDefinition()
    : m_angle()
    , m_baseX()
    , m_baseY()
    , m_offsetX()
...
inline
OdGiHatchPatternDefinition::OdGiHatchPatternDefinition()
  : mpImp(new OdGiImpHatchPatternDefinition())
{

}

inline
OdGiHatchPatternDefinition::OdGiHatchPatternDefinition(const OdGiHatchPatternDefinition &def)
  : mpImp(new OdGiImpHatchPatternDefinition())
{
  mpImp->m_angle = def.mpImp->m_angle;
  mpImp->m_baseX = def.mpImp->m_baseX;
  mpImp->m_baseY = def.mpImp->m_baseY;
  mpImp->m_offsetX = def.mpImp->m_offsetX;
  mpImp->m_offsetY = def.mpImp->m_offsetY;
  mpImp->m_dashList = def.mpImp->m_dashList;
}

inline
OdGiHatchPatternDefinition::OdGiHatchPatternDefinition(double angle,
  double baseX,
  double baseY,
  double offsetX,
  double offsetY,
  ...
  : mpImp(new OdGiImpHatchPatternDefinition())
{
  ...
  mpImp->m_baseX = baseX;
  mpImp->m_baseY = baseY;
  mpImp->m_offsetX = offsetX;
  mpImp->m_offsetY = offsetY;
  mpImp->m_dashList = dashList;
}
...
inline
double OdGiHatchPatternDefinition::angle() const
{
  return mpImp->m_angle;
}

inline
double OdGiHatchPatternDefinition::baseX() const
{
  return mpImp->m_baseX;
}

inline
double OdGiHatchPatternDefinition::baseY() const
{
  return mpImp->m_baseY;
}

inline
double OdGiHatchPatternDefinition::offsetX() const
{
  return mpImp->m_offsetX;
}

inline
double OdGiHatchPatternDefinition::offsetY() const
{
  return mpImp->m_offsetY;
}

inline
const OdGeDoubleArray& OdGiHatchPatternDefinition::dashList() const
{
  return mpImp->m_dashList;
}

inline
OdGiHatchPatternDefinition & OdGiHatchPatternDefinition::operator = (const OdGiHatchPatternDefinition &def)
{
  return *this;
}
...
class FIRSTDLL_EXPORT OdGiHatchPattern : public OdGiFill
{
  protected:
    ...
    OdArray<OdGiHatchPatternDefinition> m_patternDefArr;
  ...
  public:
    ...
    */
    OdGiHatchPattern();
    OdGiHatchPattern(const OdArray<OdGiHatchPatternDefinition>& patternDefArr);
    const OdArray<OdGiHatchPatternDefinition>& patternLines() const;
    ...
    */
    const OdHatchPattern &patternLines(ODSYSDEF) const;
    ...
    /** \details
      ...
      a reference to the OdHatchPattern that represents an array of pattern lines.
    */
    OdHatchPattern &patternLines(ODSYSDEF);
...
inline
OdGiHatchPattern::OdGiHatchPattern(const OdArray<OdGiHatchPatternDefinition>& patternDefArr)
  : m_patternDefArr(patternDefArr)
{

}

inline
const OdArray<OdGiHatchPatternDefinition>& OdGiHatchPattern::patternLines() const
{
  return m_patternDefArr;
}

inline
const OdHatchPattern &OdGiHatchPattern::patternLines(ODSYSDEF) const
{
  return m_aHatchPattern;
}

inline
OdHatchPattern &OdGiHatchPattern::patternLines(ODSYSDEF)
{
  return m_aHatchPattern;
}
...
inline
OdResult OdGiHatchPattern::copyFrom(const OdRxObject* pSource)
{
  OdGiHatchPatternPtr pSrcFill = OdGiHatchPattern::ODSYSCALL1(cast, pSource);
  if (!pSrcFill.isNull())
  {
    ...
    setPatternLines(pSrcFill->ODSYSCALL0(patternLines));
    return OdGiFill::copyFrom(pSource);
  }
...
bool OdGiHatchPattern::operator ==(const OdGiFill& fill) const
{
  if (isA() == fill.isA())
  {
    if (OdEqual(deviation(), fill.deviation()))
    {
      ...
      if (ODSYSCALL0(patternLines).size() == pSecond->ODSYSCALL0(patternLines).size() &&
      ...
      {
        for (OdUInt32 nPat = 0; nPat < ODSYSCALL0(patternLines).size(); nPat++)
        {
          const OdHatchPatternLine &pl1 = ODSYSCALL0(patternLines).getPtr()[nPat];
          const OdHatchPatternLine &pl2 = pSecond->ODSYSCALL0(patternLines).getPtr()[nPat];
