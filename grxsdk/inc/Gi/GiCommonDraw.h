...
typedef enum
{
  ...
  kOdGiForExplode                  ,  // Regenerating for explode.  
  kOdGiSaveWorldDrawForProxy       ,  // Regenerating for proxy. 
...
typedef void(*OdGiWorldSegmentCallback)(const OdGePoint3d *);
...
public:
  ...
  virtual OdBool isPsOut() const { return kFalse; }

  virtual GcGiGraphicsKernel &graphicsKernel() { static GcGiGraphicsKernel* kernel=NULL; return *kernel; }

  virtual OdCmEntityColor effectiveColor() const { return OdCmEntityColor(); }
  virtual OdDb::LineWeight byBlockLineWeight() const { return OdDb::kLnWt000; }
  virtual OdDbObjectId byBlockPlotStyleNameId() const { return OdDbObjectId(); }
  virtual OdDbObjectId byBlockMaterialId() const { return OdDbObjectId(); }
  ...
  virtual OdBool supportsFastRenderBuffer() const { return kFalse; }
  virtual OdBool supportsNativePointCloudRender() const { return kFalse; }

  virtual OdGiContextImp *getImpPtr() { return nullptr; }
  virtual const OdGiContextImp *getConstImpPtr() const { return nullptr; }

protected:
  virtual OdGiWorldSegmentCallback wSegTaker() const { return nullptr; }
  virtual int getTrueTypeBoundingBox(OdGePoint3d const &, const wchar_t *,
    int, TextParams const *, OdGePoint3d *, float *) { return 0; }

  virtual int getTrueTypeTextExtents(OdChar const *, Charset, double,
    int, int, int, short, short, double, double, OdGePoint2d &, OdGePoint2d &) { return -1; }

  virtual int setTrueTypeTransform(const OdGePoint3d &, const OdGeVector3d &, double, double, double, bool, bool) { return 0; }

  virtual int drawTrueTypeText(const OdChar *, double, int, bool, const TextParams &, Charset) { return 0; }
  virtual int restoreTrueTypeTransform() { return 0; }
  virtual void bypassHiddenLineRemoval() { }
  virtual int setTextStyle(int) { return 0; }
  virtual OdFontHandle* loadTrueTypeFont(const OdChar *, bool, bool, Charset, int, FontInfo &) { return nullptr; }
...
public:
  ...
  */
  ...
  virtual bool isOsnaping() const;
  virtual bool isEdgeSelect() const;
...
public:
  ...
  */
  ...
  virtual OdGiRegenType regenOdType() const
  {
    return regenType();
  }
  ...
  */
  ODRX_SEALED_VIRTUAL OdGiGeometry& rawGeometry(ODSYSDEF) const ODRX_SEALED
  {
    return *rawGeometry();
  }
...
{
protected:
  OdGiDrawableTraits* m_pTraits;
  bool m_bSaved;
public:
  OdGiSelectionGeomState(OdGiDrawableTraits* pTraits, bool bOn) : m_pTraits(pTraits)
  {
    if (m_pTraits)
    {
