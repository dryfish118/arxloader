...
class OdDbImpAssocGeomDependency;
...
public:
  ...
  */
  ...
  virtual OdDbAssocPersSubentId* createNewPersSubent(OdDbEntity*               pEntity,
                                                   OdDbDatabase*               pDatabase,
                                                   const OdDbCompoundObjectId& compId,
                                                   const OdDbSubentId&         subentId)
  {
    return new OdDbAssocSimplePersSubentId(subentId);
  }

  virtual OdResult releasePersSubent(OdDbEntity*                  pEntity,
                                   OdDbDatabase*                pDatabase,
  ...
  virtual OdResult transferPersSubentToAnotherEntity(OdDbEntity*                  pToEntity,
                                                   OdDbDatabase*                pToDatabase,
                                                   OdDbEntity*                  pFromEntity,
                                                   OdDbDatabase*                pFromDatabase,
                                                   const OdDbAssocPersSubentId* pPersSubentId)
  {
    return eOk;
  }

  virtual OdResult makePersSubentPurgeable(OdDbEntity*                  pEntity,
                                         OdDbDatabase*                pDatabase,
                                         const OdDbAssocPersSubentId* pPerSubentId,
                                         bool                         yesNo)
  {
  ...
  */
  ...
  virtual OdResult getAllSubentities(const OdDbEntity*      pEntity,
                                    const OdRxClass*       pSubentClass,
                                    OdArray<OdDbSubentId>& allSubentIds)
  {
    return eNotImplemented;
  }

  virtual OdResult getSubentityGeometry(const OdDbEntity*   pEntity,
                                    const OdDbSubentId& subentId,
                                    OdGeEntity3d*&      pSubentityGeometry);
  ...
  */
  ...
  virtual OdResult setSubentityGeometry(OdDbEntity*         pEntity,
                                        const OdDbSubentId& subentId,
                                        const OdGeEntity3d* pNewSubentityGeometry);

  virtual OdResult getFaceSilhouetteGeometry(OdDbEntity*     pEntity,
                                        OdRxObject*                  pContext,
                                        const OdDbSubentId&          faceSubentId,
                                        const OdGeMatrix3d*          pEntityTransform,
                                        class OdDbGeometryProjector* pGeometryProjector,
                                        OdArray<int>&                transientSilhIds,
                                        OdArray<OdGeCurve3d*>&       silhCurves)
  {
    silhCurves.removeAll();
    return eNotImplemented;
  }

  virtual OdDbAssocPersSubentId* createNewPersFaceSilhouetteId(OdDbEntity*  pEntity,
    OdDbDatabase*                pDatabase,
    OdRxObject*                  pContext,
    const OdDbCompoundObjectId&  compId,
    const OdDbSubentId&          faceSubentId,
  ...
  virtual OdResult getTransientFaceSilhouetteIds(OdDbEntity*               pEntity,
    OdDbDatabase*                pDatabase,
    OdRxObject*                  pContext,
    const OdDbSubentId&          faceSubentId,
    const OdGeMatrix3d*          pEntityTransform,
    class OdDbGeometryProjector* pGeometryProjector,
    const OdDbAssocPersSubentId* pPersSilhId,
    OdArray<int>&                transientSilhIds)
  {
