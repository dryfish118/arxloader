...
public: 
    ...
    */
    explicit OdDbAssocNetwork(OdDbAssocCreateImpObject createImpObject = kOdDbAssocCreateImpObject);
...
#if 1 // TODO: Nothing complex but needs time
...
public:
  ...
  {
    if (m_pNetwork != NULL && 0 <= m_Index && m_Index < m_pNetwork->getActions().length())
    {
      return m_pNetwork->getActions()[m_Index];
    }
    else
    {
#ifdef ASSERT
  ...
  bool moveNext()
  {
    return m_pNetwork != NULL ? ++m_Index < m_pNetwork->getActions().length() : false;
  }

  void reset() { m_Index = -1; }

private:
  const OdDbAssocNetwork* const m_pNetwork;
  int                           m_Index;
};
