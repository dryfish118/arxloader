...
public:
	...
	*/
  explicit OdDbAssocEdgeFilletActionBody(OdDbAssocCreateImpObject createImpObject = kOdDbAssocCreateImpObject);
  ...
  OdResult setInputEdges(const OdDbFullSubentPathArray& filletEdges);
  OdResult setRadius(double radius, const OdString& expression = OdString(), 
    const OdString& evaluatorId = OdString());
  double radius(OdString& expression = dummyString(), OdString& evaluatorId = dummyString()) const;
  static OdResult createInstance(const OdDbFullSubentPathArray& filletEdges,
    double radius, bool bEnabled, OdDbObjectId& createdActionId);
