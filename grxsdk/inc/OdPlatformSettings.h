...
#if defined(__APPLE__) || ((__GLIBC__) && (__GLIBC__ > 2)) || (__GLIBC__ == 2 && __GLIBC_MINOR__ > 10)
...
#endif
#if defined(ODA_WINDOWS)
typedef wchar_t OdChar16_t;
#elif __APPLE__
typedef __CHAR16_TYPE__ OdChar16_t;
#else
typedef char16_t OdChar16_t;
#endif

#define OdBool_is_bool 1
#if defined(OdBool_is_bool)
typedef bool OdBool;
...
#define OD_BOOL(a) ((a) ? kTrue : kFalse)

//#define OdSize_is_size_t 1
#if defined(OdSize_is_size_t)
typedef size_t OdSize;
#else
typedef int OdSize;
#endif
...
typedef __w64 int          OdLongPtr;
typedef __w64 unsigned int OdULongPtr;
//
typedef __w64 int           OdIntPtr;
typedef __w64 unsigned int  OdUIntPtr;

#else

typedef __int64             OdLongPtr;
typedef unsigned __int64    OdULongPtr;
typedef __int64             OdIntPtr;
typedef unsigned __int64    OdUIntPtr;

#endif

typedef OdIntPtr OdGsMarker; // Integer able to store a pointer (32 or 64 depending on platform)

typedef OdLongPtr OdHWnd;

typedef OdLongPtr OdIntDbId;
