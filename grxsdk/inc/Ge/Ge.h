...
struct OdGe
{
  ...
  enum EntityId 
  {
    ...
    kDummy8,
    kDummy9,
    kLWPolyine3d = 0x1000	//LWPolyline对应的多段线,内部使用
  };

  enum ExternalEntityKind 
  {
    kAcisEntity              = 0, // External Entity is an ACIS entity
	kExternalEntityUndefined = 1, // External Entity is an OdGeCurve3d* entity
    kGe3dSurfaceEntity       = 2, // External Entity is an OdGeSurface3d* entity
	kGe3dCurveEntity,      // External Entity is undefined. 
    kBimEntity
  }; 
  ...
  */
  ...
  typedef KnotParameterization OdGeKnotParameterization;
};
...
#define DECLARE_GE_RTTI() \
  virtual OdBool isKindOf (OdGe::EntityId entType) const;\
  virtual OdGe::EntityId type() const

#define DEFINE_GE_RTTI(Class,Parent,TYPE) \
  OdBool Class::isKindOf(OdGe::EntityId entType) const\
  {\
