...
#ifdef ASSERT
#define OD_ARRAY_ASSERT ASSERT
#elif defined assert
#define OD_ARRAY_ASSERT assert
#elif defined _ASSERTE
#define OD_ARRAY_ASSERT _ASSERTE
#else
#define OD_ARRAY_ASSERT(T)
#endif

#define ODARRAY_GROWTH_THRESHOLD 0x10000
...
template <class T> void OdArrayValidateParams(bool bSameBuffer,
...
public:
  ...
  {
    OdArrayValidateParams<T>(false, pDest, nBufLen, pSource, nCount);
    if (nCount > 0)
  ...
  static void moveItems(T* pDest, size_t nBufLen, T * pSource, size_t nCount,
    bool bSameBuffer)
  {
    OdArrayValidateParams<T>(bSameBuffer, pDest, nBufLen, pSource, nCount);
    if (nCount > 0) {
      if (bSameBuffer)
...
struct OdArrayItemCopierSelector;

template<typename T>
struct OdArrayItemCopierSelector<T, false>
{
  typedef OdObjectsAllocator<T> allocator;
};

template<typename T>
struct OdArrayItemCopierSelector<T, true>
{
  typedef OdObjectsAllocator<T> allocator;
};
/** \details
...
public:
  ...
  typedef T Type;
  typedef A Allocator;
  enum { eUsesMemCopy = std::is_same<A, OdMemoryAllocator<T> >::value };
private:
  ...
  public:
    ...
    inline void reallocate(OdArray* pArray, size_type nNewLen )
    {
      ...
      {
        if((int)nNewLen > pArray->physicalLength())
        {
          if ( !_may_use_realloc )
          {
            m_pBuffer->release();
  ...
  void copy_before_write(size_type len, bool may_use_realloc = false )
  {
    if( referenced() )
      copy_buffer(len);
    else if ( (int)len > physicalLength() )
  ...
  void copy_buffer( size_type len, bool may_use_realloc = false, bool force_size = false )
  {
    ...
    if ( !force_size )
    {
      if(nGrowBy > 0)
      {
        if (len2 > pOldBuffer->m_nAllocated && (int)len2 > nGrowBy)
          len2 *= 2;
        else
          len2 += nGrowBy;
...
public:
  ...
  */
  ...
  {
    if(physicalLength() < (int)reserveLength)
    { 
  ...
  */
  ...
  { 
    ...
    if (!referenced())
    {
      if (len == (size_type)physicalLength())
      {
  ...
  */
  iterator insert(
    ...
    const T& value = T())
  {
    int index = (int)(before - begin_const());
  ...
  */
  ...
  /** \details
    ...
    \param arrayIndex [in]  Array index. 
  */
  const T& at(int arrayIndex) const
  { 
    assertValid(arrayIndex); 
    return *(data() + arrayIndex); 
  }
  ...
  */
  ...
  OdArray<T, A>& appendMove(OdArray<T, A>& array)
  {
    return *this;
  }
  ...
  */
  ...
  OdArray<T, A>& insertAtMove(int index, T& value)
  {
    return *this;
  }
  ...
  */
    ...
    int endIndex)
  {
    ...
    T* pData = data();
    ++endIndex;
    int n2remove = endIndex - startIndex;
...
protected:
  ...
  void copyOtherIntoThis(const OdArray<int, OdArrayItemCopierSelector<int, std::is_trivial<int>::value>::allocator >& otherArray)
  {
  }
  void moveOtherIntoThis(OdArray<int, OdArrayItemCopierSelector<int, std::is_trivial<int>::value>::allocator >& otherArray)
  {
  }
  bool isValid(int num1) const
  {
    return num1 >= 0 && num1 < logicalLength();
  }
