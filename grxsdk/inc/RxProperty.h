...
protected:
  ...
  */
  ...
  explicit OdRxProperty(const OdChar* name, const OdRxValueType& type, const OdRxObject* owner = NULL) throw();
...
protected:
  virtual OdRxValueIteratorPtr subNewValueIterator(ODSYSDEF, const OdRxObject* pO) const;
  virtual OdRxValueIterator* subNewValueIterator(const OdRxObject* pO) const;
  virtual OdResult subTryGetCount(const OdRxObject* pO, int& count) const;
  using OdRxMember::init;
  OdRxCollectionProperty(const OdChar* name, const OdRxValueType& type, const OdRxObject* owner = NULL);
...
protected:
  ...
  OdRxIndexedProperty(const OdChar* name, const OdRxValueType& type, const OdRxObject* owner = NULL);
...
protected:
  virtual OdResult subGetValue(const OdRxObject* pO, const OdChar* key, OdRxValue& value) const { return eNotApplicable; }
  virtual OdResult subSetValue(OdRxObject* pO, const OdChar* key, const OdRxValue& value) const { return eNotApplicable; }
  static OdRxCollectionPropertyPtr createObject(const OdString& name, const OdRxValueType& type, const OdRxObject* owner = NULL);
  OdRxDictionaryProperty(const OdChar* name, const OdRxValueType& type, const OdRxObject* owner = NULL);
...
struct Object ## Name ## Property : OdRxIndexedProperty {\
  ...
  virtual OdRxValueIteratorPtr subNewValueIterator(ODSYSDEF, const OdRxObject* pO) const ODRX_OVERRIDE;\
...
#define ODRX_DECLARE_RX_PROPERTY_GET_BY_VAL(Object, ObjGetMemberName, Type) \
  virtual OdResult subGetValue(const OdRxObject* pO, OdRxValue& value) const ODRX_OVERRIDE\
  {\
    Object ## Ptr pObj = Object::ODSYSCALL1(cast, pO);\
