...
const OdChar ODDB_NAMEDPATH_DICTIONARY[] = __OD_T("ACAD_NAMEDPATH");

SCENEOE_EXPORT OdResult gcdbGetNamedPathDictionary(OdDbDatabase* pDb, OdDbDictionary*& pDict, OdDb::OpenMode mode, bool createIfNonExist);
SCENEOE_EXPORT OdResult gcdbGetNamedPathDictionary(OdDbDatabase* pDb, OdDbDictionary*& pDict, OdDb::OpenMode mode);


class OdGeCurve3d;
class OdDbImpNamedPath;
...
protected:
  OdDbNamedPath(OdDbImpNamedPath* pImp);
  OdDbImpNamedPath* imp() const;

private:
  friend class OdDbImpNamedPath;
  OdDbImpNamedPath* m_Imp;
};
...
class SCENEOE_EXPORT OdDbCurvePath : public OdDbNamedPath
{
  public:
    ...
    OdResult setGeometry(const OdDbObjectId &curveId);
...
class SCENEOE_EXPORT OdDbPointPath : public OdDbNamedPath
{
  public:
    ...
    OdResult setGeometry(const OdGePoint3d &point);
