...
public:
  ...
  */
  OdDbAssoc2dConstraintGroup(const OdGePlane& plane = OdGePlane::kXYPlane);

  ODRX_SEALED_VIRTUAL OdResult audit(OdDbAuditInfo* pAuditInfo) ODRX_OVERRIDE;

  /** \details
    Sets the work plane to associate it with this planar constraint group.
  */
  void setWorkPlane(const OdGePlane& plane);
  ...
  */
  OdResult getConstrainedGeometry(const OdDbAssocGeomDependency* pGeomDependency,
                                  OdConstrainedGeometry*&   pConsGeom,
                                  OdDb::ImplicitPointType* pPtType = NULL,
                                  int defPtIndex = -1,
                                  bool bCreateArcLineMid = false);
...
public:
  virtual ~OdAutoConstrainEvaluationCallback()
  {
