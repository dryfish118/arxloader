...
struct IOdRxReferenceType
{
  ...
  */
  ...
  ///
  virtual OdRxObject* beginDereferencing(const OdRxValue& value, OpenMode mode) const = 0;
  ...
  ///
  virtual OdResult endDereferencing(OdRxObject* pO) const = 0;
  ...
  /** \details
    ...
    released, as usual.
  */
  virtual OdRxObjectPtr dereference(const OdRxValue& value, OpenMode mode) const
  {
    //ODA TODO
    static OdRxObjectPtr test;
    return test;
  }
};
...
public:
  ...
  */
  virtual OdRxObject* create() const ODRX_OVERRIDE;
  ...
  */
  ...
  int toString(const void* instance, OdChar* buffer, size_t sizeInACHARs, StringFormat format) const
  {
    ODA_ASSERT(instance != NULL);
    if (instance == NULL)
      return -1;
    ODA_ASSERT((sizeInACHARs == 0) == (buffer == NULL));
...
private:
  ...
  virtual int subToString(const void* instance, OdChar* buffer, size_t sizeInGCHARs, StringFormat format) const = 0;
