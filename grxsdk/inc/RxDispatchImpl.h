...
protected:
  ...
  public:
    ...
    const OdChar* key() const { return m_pOwner->keyAt(id()); }
...
public:
  ...
  const OdChar* keyAt(OdUInt32 id) const { return propertiesInfo()->keyAt(id); }
  ...
  OdBool isSorted() const { return propertiesInfo()->isSorted(); }
