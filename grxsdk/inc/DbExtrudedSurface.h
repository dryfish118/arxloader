...
public:
  ...
  */
  virtual OdResult createExtrudedSurface(OdDbEntity* pSweepEnt,
    const OdGeVector3d& directionVec, OdDbSweepOptions& sweepOptions);
  virtual OdResult createExtrudedSurface (OdDbEntity* pSweepEnt,
    const OdGeVector3d& directionVec, OdDbSweepOptions& sweepOptions, const OdStreamBuf *pSat);

  /** \details
    Returns the sweep entity used to create the extruded surface.
  */
  OdDbEntityPtr getSweepEntity () const;
  TOOLKIT_EXPORT OdDb3dProfile* sweepProfile() const;
  ...
  */
  OdResult setSweepVec(const OdGeVector3d& sweepVec);
  ...
  */
  ...
  /** \details
    ...
    \remarks
     ...
     for other modelers. 
  */
  OdResult setHeight(double height);
